...
import { Button, ButtonCard, Divider, AppHeader } from '../../Component';
import styles from './styles';
import {useFocusEffect} from '@react-navigation/native';
import {useSelector, useDispatch} from 'react-redux';
import { loadScore } from '../../redux/actions/GameActions';

const SportsQuiz = (props) => {
  const {navigation} = props;
  const [score, setScore] = useState(0);
  const [resetTimer, setResetTimer] = useState(false);
  const [success, setSuccess] = useState(null);
  const [highScore, setHighScore] = useState(useSelector(state => state.main.sports));
  const TimeLimit = 10;
  ...
  const timeout = () => {
    console.log({SportsHighScore: highScore})
    if (problem === 10) {
      if(score > highScore)
      {
        dispatch(loadScore(score, 1))
        .then((response) => {
  ...
  return (
    <View style={styles.container}>
      <SafeAreaView />
      <AppHeader title={'weRplay'} subTitle={' Sports '} />
