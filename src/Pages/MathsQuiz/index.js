...
const MathsQuiz = (props) => {
  const {navigation} = props;
  const [score, setScore] = useState(0);
  const [resetTimer, setResetTimer] = useState(false);
  const [success, setSuccess] = useState(null);
  const [highScore, setHighScore] = useState(useSelector(state => state.main.maths));
  const TimeLimit = 10;
  ...
  return (
    <View style={styles.container}>
      <SafeAreaView />
      <AppHeader title={'weRplay'} subTitle={' Maths '} />
